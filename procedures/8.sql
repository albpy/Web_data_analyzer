-- PROCEDURE: public.get_sp_otb_commit_drilldown(text, text[], text[], text[], text[], text[], text[], text[], text, text, text[], text[])

-- DROP PROCEDURE IF EXISTS public.get_sp_otb_commit_drilldown(text, text[], text[], text[], text[], text[], text[], text[], text, text, text[], text[]);
...
BEGIN
	...
	IF columnsnames IS NOT NULL AND array_length(columnsnames, 1) IS NOT NULL THEN
	    FOR i IN 1..array_length(columnsnames, 1) LOOP
	        where_clause := where_clause || '"'|| columnsnames[i] || '"' || ' = ' || quote_literal(columnvalues[i]);
  ...
  sql_query := 'CREATE TABLE otb_drilled_out AS SELECT ' ||
        ...
        sql_query :=    sql_query || ' FROM ' || table_name_ || ' ' || where_clause || ' ' ||
                 		CASE WHEN group_clause <> '' THEN 'GROUP BY ' || group_clause ELSE '' END;
...
END
$BODY$;
ALTER PROCEDURE public.get_sp_otb_commit_drilldown(text, text[], text[], text[], text[], text[], text[], text[], text, text, text[], text[])
    OWNER TO postgres;
