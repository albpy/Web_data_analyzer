-- PROCEDURE: public.get_sp_otb_get_filter_1(text)

-- DROP PROCEDURE IF EXISTS public.get_sp_otb_get_filter_1(text);

CREATE OR REPLACE PROCEDURE public.get_sp_otb_get_filter_1(
	IN table_name_ text)
LANGUAGE 'plpgsql'
AS $BODY$
	DECLARE 
...
BEGIN
    ...
    -- IF array_length(sub_fam, 1) IS NOT NULL THEN
        ...
        SELECT DISTINCT "sub_family" FROM item_counter  WHERE "sub_family" IS NOT NULL; --WHERE "SubFamily" = ANY(sub_fam);
    -- END IF;
    ...
    -- IF array_length(categ, 1) IS NOT NULL THEN
        ...
        SELECT DISTINCT "category_name" FROM item_counter WHERE "category_name" IS NOT NULL; --WHERE "Category" = ANY(categ);
    -- END IF;
    ...
    -- IF array_length(sub_categ, 1) IS NOT NULL THEN
        ...
        SELECT DISTINCT "sub_category" FROM item_counter WHERE "sub_category" IS NOT NULL; --WHERE "SubCategory" = ANY(sub_categ);
    -- END IF;

    -- IF array_length(extended_sub_categ, 1) IS NOT NULL THEN
        INSERT INTO otb_sub_filter ("Extended_sub_category")
        SELECT DISTINCT "extended_sub_category" FROM item_counter WHERE "extended_sub_category" IS NOT NULL; --WHERE "ExtendedSubCategory" = ANY(extended_sub_categ);
    -- END IF;

    -- IF array_length(sub_category_supp, 1) IS NOT NULL THEN
        INSERT INTO otb_sub_filter ("Sub_Category_supplier")
        SELECT DISTINCT "sub_category_supplier" FROM item_counter WHERE "sub_category_supplier" IS NOT NULL; --WHERE "SubCategorySupplier" = ANY(sub_category_supp);
    -- END IF;
	progress := 13;
...
END;
$BODY$;
ALTER PROCEDURE public.get_sp_otb_get_filter_1(text)
    OWNER TO postgres;
