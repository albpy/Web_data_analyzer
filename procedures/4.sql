-- PROCEDURE: public.get_sp_otb_itemwise_calculations_1(date, date)

-- DROP PROCEDURE IF EXISTS public.get_sp_otb_itemwise_calculations_1(date, date);

CREATE OR REPLACE PROCEDURE public.get_sp_otb_itemwise_calculations_1(
...
-- 		data = data.with_columns(retail_value_including_markdown = pl.col("supply_retail_value")/(1-(pl.col('markdown_percent')/100)))
					...
					retail_value_including_markdown = ((budget_cost - (COALESCE(budget_cost,0) * ("Logistic%" / 100))) / (1 - ("FirstMargin_percent"/100)))/(1-(markdown_percent/100)),
...
-- 		data = data.with_columns(PurchaseRetailValueatGrossSale = pl.col('retail_value_including_markdown')/(pl.col('proposed_sellthru_percent')/100))
					-- "PurchaseRetailValueatGrossSale" = (((budget_cost - (COALESCE(budget_cost,0) * ("Logistic%" / 100))) / (1 - ("FirstMargin_percent"/100)))/(1-(markdown_percent/100)) / (proposed_sellthru_percent/100)),
--data = data.with_columns(otb_retail_value_at_gross_sale = (pl.col('PurchaseRetailValueGrossSale')-((pl.col('DisplayItemQty')-pl.col('DisplayItemValue')-pl.col('COR_EOLStock_value')))).replace({np.inf:0, -np.inf:0}).fill_nan(0).fill_null(0))
					otb_retail_value_at_gross_sale = "PurchaseRetailValueatGrossSale" -("DisplayItemQty" - "DisplayItemValue" - "COR_EOLStock_value"),					
--	 data = data.with_columns(OTBorPurchaseCost=(pl.col('PurchaseRetailValueatGrossSale')*(1-(pl.col('FirstMargin_percent')/100)).replace({-np.inf:0, np.inf:0}).fill_nan(0).fill_null(0)))

					"OTBorPurchaseCost" = ("PurchaseRetailValueatGrossSale" -("DisplayItemQty" - "DisplayItemValue" - "COR_EOLStock_value")) * (1-("FirstMargin_percent"/100)),
-- df = df.with_columns(OTBquantity=(pl.col('OTBorPurchaseCost')/((pl.col('budget_cost'))/(pl.col('budget_qty')))))
					-- "OTBquantity" = ("PurchaseRetailValueatGrossSale" * (1-("FirstMargin_percent"/100)))/(budget_cost)/(budget_qty),
					"OTBquantity" = CASE
...
-- df = df.with_columns(StockatRetailPrice = (pl.col('initial_average_retail_price') * pl.col('stock_on_hand_qty')).replace({np.inf:0,-np.inf:0}).fill_nan(0).fill_null(0))
					...
					"TYForecast" = COALESCE("StockatRetailPrice", 0) - COALESCE("DisplayItemValue", 0) - COALESCE("COR_EOLStock_value", 0),
					"ACT_FCT" = 
						CASE 
...
$BODY$;
ALTER PROCEDURE public.get_sp_otb_itemwise_calculations_1(date, date)
    OWNER TO postgres;
