-- PROCEDURE: public.get_sp_otb_root_frame_calc_1()

-- DROP PROCEDURE IF EXISTS public.get_sp_otb_root_frame_calc_1();

CREATE OR REPLACE PROCEDURE public.get_sp_otb_root_frame_calc_1(
	)
LANGUAGE 'plpgsql'
AS $BODY$
-- DECLARE 
...
ADD COLUMN otb_retail_value_at_gross_sale NUMERIC(20, 4) DEFAULT 0,
ADD COLUMN adjusted_sellthru_percent NUMERIC(20, 4) DEFAULT 0;
...
--      markdown_percent = (((initial_average_retail_price * "sold_qty_ly") - (final_price * "sold_qty_ly")) / COALESCE(NULLIF((initial_average_retail_price * "sold_qty_ly"),0),1)) * 100 ,
	...
	-- supply_retail_value =  (budget_cost - ((COALESCE(budget_cost,0) * ("Logistic%" / 100)))/(1-((budget_amount -  budget_cost - (COALESCE(budget_cost,0) * ("Logistic%" / 100))) / COALESCE(NULLIF(budget_amount,0),1)) * 100)), 

	-- df = df.with_columns(retail_value_including_markdown = pl.col("supply_retail_value")/(1-pl.col('markdown_percent')))
	-- retail_value_including_markdown = (budget_cost - ((COALESCE(budget_cost,0) * ("Logistic%" / 100)))/(1-((budget_amount -  budget_cost - (COALESCE(budget_cost,0) * ("Logistic%" / 100))) / COALESCE(NULLIF(budget_amount,0),1)) * 100)) / (1-NULLIF((ly_customer_disc/(ly_customer_disc + net_sales_ly) * 100), 0)),
...
CALL otb_purchase_retail_value_at_gross_sale('ra_sales_item_stock_kpi_joined', 'markdown_percent');

END;
$BODY$;
ALTER PROCEDURE public.get_sp_otb_root_frame_calc_1()
    OWNER TO postgres;
