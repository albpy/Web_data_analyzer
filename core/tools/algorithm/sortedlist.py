...
class SortedList(MutableSequence):
    ...
    def key(self):
        """
            ...
            list compares values directly so the key function is none.
        """
        return None
    ...
    def append(self, value):
        raise NotImplementedError('overridden with ``add(value)`` instead')


    def extend(self, values):
        raise NotImplementedError('overridden with  ``update(values)`` instead')


    def insert(self, index, value):
        raise NotImplementedError('overridden with ``sl.add(value)`` instead')
