"""
Contains the :class:`base class <indexer.storages.Storage>` for storages and
implementations.
"""
...
class JSONStorage(Storage):
    ...
    def read(self) -> Optional[Dict[str, Dict[str, Any]]]:
        ...
        if not size:
            # File is empty, so we return ``None`` so Indexer can properly
...
class RedisStorage(Storage):
    """
    Store the data in a Redis database.
    """

    def __init__(self, host=os.getenv("REDIS_HOST"), port=6379, db=0, password=os.getenv("REDIS_PASSWORD"), key='indexer', **kwargs):
        """
        Create a new instance.
        
        :param host: Redis server address. Default is 'redis' for Docker Compose.
        ...
        self.key = key
        self.client = redis.StrictRedis(host=host, port=port, db=db, password=password, **kwargs)
    ...
    def read(self) -> Optional[Dict[str, Dict[str, Any]]]:
        ...
        if not raw_data:
            ...
            # Deserialize the JSON contents from Redis
            return json.loads(raw_data.decode('utf-8'))

    def write(self, data: Dict[str, Dict[str, Any]]):
        # Serialize the database state into JSON
        serialized = json.dumps(data)

        # Store the serialized data in Redis
        self.client.set(self.key, serialized)

    def close(self) -> None:
        """
        Optional: Close the Redis connection.
        """
        self.client.close()
