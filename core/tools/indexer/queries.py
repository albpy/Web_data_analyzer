...
Starting with :class:`~indexer.queries.Query` you can construct complex
queries:
...
class QueryLike(Protocol):
    ...
    2. it must have a sindex hash that will be used for query caching.
    """
...
class QueryInstance:
    ...
    :class:`~indexer.queries.Query` class acts like a query builder and
    generates :class:`~indexer.queries.QueryInstance` objects which will
    ...
    In order to be usable in a query cache, a query needs to have a sindex hash
    ...
    def __hash__(self) -> int:
        ...
        # describes this query uniquely, so we can calculate a sindex hash
...
class Query(QueryInstance):
    ...
    Allows building queries for Indexer databases. There are two main ways of
    using queries:
    ...
    def _generate_test(
        ...
        :return: A :class:`~indexer.queries.QueryInstance` object
        """
    ...
    def test(self, func: Callable[[Mapping], bool], *args) -> QueryInstance:
        ...
        .. warning::
            ...
            may mess up the query cache that :class:`~indexer.index.Index`
            implements.
    ...
    def map(self, fn: Callable[[Any], Any]) -> "Query":
        ...
        # ... and kill the hash - callable objects can be muindex, so it's
